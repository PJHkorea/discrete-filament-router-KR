...
class DFRMacroCognitiveDialTower:
    ...
    HZ_MIN: Final[float] = 5000.0   # 최소 냉각/대기 주파수 (5 kHz)
    HZ_MAX: Final[float] = 15000.0  # 최대 부하 발전 주파수 (15 kHz)
    ...
    def __init__(self, target_temperature: float = 500.0):
        ...
        self.current_injection_hz = self.HZ_MAX  # 초기 가동 시 최대 출력 발전 모드로 셋팅
        ...
        self.current_avg_valve_ratio = 1.0       # 전 구간 평균 밸브 개도율 (1.0 = 100% 완전 개방 청정 상태)
    ...
    def dynamic_inference_injection_dial(self, current_telemetry_temp: float, grid_demand: float, avg_valve_ratio: float = 1.0) -> float:
        ...
        @details 무거운 미분이나 실시간 자석 위치 연산 지연 없이, 거시적 열에너지 상태와 
                 전력망 요구량 및 진공 밸브 마진을 바탕으로 입구 잉크젯 분사 주파수 다이얼을 하향식 변조합니다.
        """
        ...
        # 온도가 임계치(520°C 돌파)를 넘어 과열되거나, 하부 밸브가 급격히 차단되어 진공 컨덕턴스 면적이 80% 미만(avg_valve_ratio < 0.8)으로
        # 병목 정체가 발생하면, 발전스트림을 강제 끄지 않고 연료 주입 다이얼만 5 kHz 최소선으로 즉각 툭 떨어뜨림
        if self.current_plant_temp > (self.target_temp + 20.0) or self.current_avg_valve_ratio < 0.8:
            # 냉각 마진 확보 및 단위 면적당 열/진공 부하 급감 유도 (정비 중단 없는 자가 안정화 영역 진입)
            self.current_injection_hz = self.HZ_MIN
            print(f"[Level 4 🧠] ⚠️ 비상/정체 경보 감지 [온도: {self.current_plant_temp}°C, 밸브면적: {self.current_avg_valve_ratio*100:.1f}%] -> 연료 다이얼 {self.HZ_MIN/1000:.1f} kHz 최소 안정화 모드로 변조.")
        ...
        # 정상 열역학 평형 상태 하에서 전력망 부하 추종(Load Following) 시퀀스 작동
            ...
            print(f"[Level 4 🧠] ✅ 배관 온도가 안정적입니다 ({self.current_plant_temp}°C). 외부 Grid 수요({self.grid_demand_factor*100:.1f}%) 추종 출력: {self.current_injection_hz:.1f} Hz")
       ...
       async def run_cognitive_dial_loop(self, orchestrator_l3):
        ...
        while orchestrator_l3.is_running:
            ...
            print(f"\n📊 [Level 4 텔레메트리 스캔] 가동 자석 노드: {active_sectors}/16 | 인프라 실질 배기 면적 마진: {avg_valve_open*100:.1f}% | 누적 결함 감결합: {failed_history_cnt}")
            ...
            # 💡 인프라 결합 정답: 연료 변조와 함께 실시간 전역 진공 밸브 면적 계수(avg_valve_open)를 동시 인젝션하여 복합 하향식 조향 집행!
            target_hz = self.dynamic_inference_injection_dial(mock_temp, mock_grid_demand, avg_valve_ratio=avg_valve_open)
            
            # 🛡️ 버그 수정 완결: 특정 7번 섹터만 편향 감시하던 하드코딩 결함을 영구 멸절 처리하고,
            ...
            # 만약 결함 히스토리가 존재하고, 전 구간 연쇄 소프트 리셋 및 이완이 끝난 상태가 확증되었다면 정상 종료 가드 발동
            if failed_history_cnt > 0 and is_all_plant_restored:
                print(f"[Level 4 🧠] ➔ 🔄 [전 구간 수렴 확증] 사후 소산 및 10⁻⁵ Torr 진공/밸브 이완 재점화 복구 완료 확인에 따른 거시 인지 루프 동기화 정착 마감.")
                break
